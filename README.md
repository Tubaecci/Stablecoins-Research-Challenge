...
| **DAI (MakerDAO)** | Overcollateralized with crypto assets (ETH, wstETH, USDC, etc.). Smart contracts enforce collateral ratios and liquidations. Peg stability supported by arbitrage and stability fees. | Proven resilience (e.g., March 2020 crash). Generates protocol revenue via fees and liquidations. Reliance on USDC as collateral reduces independence and increases exposure to centralized risk. | Semi-decentralized. Governed by MakerDAO token holders, but reliance on USDC and governance token concentration introduce centralization risks. |
